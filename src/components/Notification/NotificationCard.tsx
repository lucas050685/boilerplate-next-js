...
export function NotificationCard({ notification }: { notification: Notification }){
  ...
  const defaultActionClass = "whitespace-nowrap opacity-50 hover:opacity-100 border p-2 rounded cursor-pointer min-w-[30px] h-[30px] flex justify-center items-center";
  ...
  const renderActions = () => {
    return notification.actions?.map((action) => {
      ...
      return <div className={classNames} key={action.label} onClick={() => action.callback(notification.id ?? '')} >{action.label}</div>
    });
  };

  return <div className={selectedClassName}>
    {icons[selectedType] && <div className='text-lg'>
      {icons[selectedType]}
    </div>}
